...
export default async function handler(req, res) {
	...
	); // capture date and time now then convert it to local time for comparison
	const { currentSkyColor, timeOfDay } = await getCurrentSkyColors(
		currentLocalDateTime
	);
	console.log(currentSkyColor, timeOfDay);

	res.status(200).json({
		currentSkyColor: currentSkyColor,
		timeOfDay: timeOfDay,
	});
}
...
async function getCurrentSkyColors(currentDateTime) {
	...
	try {
		const astro_twilight_begin = new Date(
			new Date(sunTimes.astronomical_twilight_begin).toLocaleString(
				'en-US',
		...
		const sunrise = new Date(
			new Date(
				sunTimes.sunrise.toLocaleString('en-US', {
					timeZone: 'America/New_York',
				})
			)
		);
		const sunset = new Date(
			new Date(sunTimes.sunset).toLocaleString('en-US', {
				timeZone: 'America/New_York',
			})
		);
		const astro_twilight_end = new Date(
			new Date(sunTimes.astronomical_twilight_end).toLocaleString(
				'en-US',
				{ timeZone: 'America/New_York' }
			)
		);
		...
		if (
			currentDateTime > astro_twilight_begin &&
			currentDateTime < sunrise
		) {
			console.log('currently dawn');
			currentSkyColor = skyColors.dawn;
			timeOfDay = 'dawn';
		} else if (
			currentDateTime > sunset &&
			currentDateTime < astro_twilight_end
		) {
		...
		console.log(
			'current sky color and time of day: ',
			currentSkyColor,
			timeOfDay
		);
